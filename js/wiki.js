...
(function (window, document) {
  ...
  async function init() {
    ...
    try {
      ...
      renderState(`Không tải được dữ liệu Wikipedia D8: ${error.message}`);
      updateStatus();
    }
  }
  ...
  function bindEvents() {
    ...
    [controls.sort, controls.dance, controls.rate].forEach(control => control?.addEventListener("change", () => applyFilters(true)));
    controls.clear?.addEventListener("click", clearFilters);
    document.addEventListener("change", event => {
      if (event.target.matches("#wikiLevelFilter,#wikiTypeFilter,[data-mina-level-filter],[data-mina-type-filter]")) applyFilters(true);
    });
  ...
  function applyFilters(resetVisible = false) {
    ...
    const dance = String(controls.dance?.value || "");
    const rate = String(controls.rate?.value || "");
    ...
    filteredSkills = allSkills.filter(skill => {
      const haystack = engine.normalizeText([
        skill.id, skill.name, skill.alias, skill.style, skill.level, skill.type,
        skill.danceName, skill.rarity, skill.bpm, skill.quality,
        skill.description, ...(skill.tags || [])
      ].join(" "));
      return (!query || haystack.includes(query))
        && (!dance || skill.danceName === dance || skill.name === dance)
        && (!rate || skill.rarity === rate || String(skill.rating) === rate)
        && (!level || String(skill.level) === level)
  ...
  function sortSkills() {
    ...
    if (mode === "rating") filteredSkills.sort((a, b) => b.rating - a.rating);
    else if (mode === "bpm") filteredSkills.sort((a, b) => bpmNumber(a.bpm) - bpmNumber(b.bpm));
    else if (mode === "id") filteredSkills.sort((a, b) => String(a.id).localeCompare(String(b.id), "vi", { numeric: true }));
    else if (mode === "newest") filteredSkills.sort((a, b) => dateNumber(b.updatedAt) - dateNumber(a.updatedAt));
  }
      ...
      card.innerHTML = `
        ...
        <div class="wiki-card-body">
          ...
          <div class="wiki-actions">
            ...
            <button type="button" class="wiki-video-btn" ${skill.youtube ? "" : "disabled"}>▶ ${skill.youtube ? "Xem video" : "Chưa có video"}</button>
          </div>
        </div>`;
  ...
  function renderLoading() {
    grid.innerHTML = Array.from({ length: 8 }, () => `
      <article class="wiki-card mina-wiki-skeleton"><div class="mina-skeleton-image"></div><div class="wiki-card-body"><div class="mina-skeleton-line short"></div><div class="mina-skeleton-line title"></div><div class="mina-skeleton-line"></div></div></article>
    `).join("");
  }
  ...
  function ensureStatus() {
    ...
    controls.tools.insertAdjacentHTML("afterend", `<div id="minaWikiStatus" class="mina-wiki-status"><span id="minaWikiResultText">Đang chuẩn bị dữ liệu...</span><span class="mina-wiki-shortcut">Nhấn <kbd>/</kbd> để tìm nhanh</span></div>`);
  }

  function updateStatus() {
    const text = document.getElementById("minaWikiResultText");
    if (text) text.textContent = `Hiển thị ${Math.min(visibleCount, filteredSkills.length)}/${filteredSkills.length} Skill • Tổng dữ liệu: ${allSkills.length}`;
  }

  function populateDanceFilter() {
    if (!controls.dance) return;
    const current = controls.dance.value;
    const first = controls.dance.querySelector('option[value=""]')?.cloneNode(true) || new Option("Tên Bước Nhảy", "");
    const values = [...new Set(allSkills.map(skill => skill.danceName).filter(Boolean))].sort((a, b) => a.localeCompare(b, "vi"));
  ...
  function clearFilters() {
    ...
    document.querySelectorAll("#wikiLevelFilter,#wikiTypeFilter,[data-mina-level-filter],[data-mina-type-filter]").forEach(control => { control.value = ""; });
    applyFilters(true);
    controls.search?.focus();
  }
  ...
  function renderLoadMore() {
    ...
    button.addEventListener("click", () => { visibleCount += 24; render(); });
    wrap.appendChild(button);
    grid.insertAdjacentElement("afterend", wrap);
  }

  function removeLoadMore() { document.getElementById("minaWikiLoadMore")?.remove(); }
  function bpmNumber(value) { const match = String(value || "").match(/\d+/); return match ? Number(match[0]) : Number.MAX_SAFE_INTEGER; }
  function dateNumber(value) { const time = Date.parse(value || ""); return Number.isFinite(time) ? time : 0; }
