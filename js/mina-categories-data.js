export const MINA_DEFAULT_CATEGORIES = [
  {
    "id": "trai-nghiem-game",
    "name": "TRẢI NGHIỆM GAME",
    "icon": "🎮",
    "children": [
      {
        "id": "share-file-skill-d8",
        "name": "Share File Skill D8",
        "icon": "📄",
        "children": []
      },
      {
        "id": "share-file-skill-d8team",
        "name": "Share File Skill D8Team",
        "icon": "📄",
        "children": []
      },
      {
        "id": "cach-quay-dung-video-anh",
        "name": "Cách Quay Dựng Video & Ảnh",
        "icon": "🎥",
        "children": []
      },
      {
        "id": "su-kien-ingame-audition",
        "name": "Sự Kiện Ingame Audition",
        "icon": "🎉",
        "children": []
      },
      {
        "id": "van-de-khac",
        "name": "Vấn Đề Khác",
        "icon": "📌",
        "children": []
      }
    ]
  },
  {
    "id": "lenh-ai-tao-anh-3d",
    "name": "LỆNH AI TẠO ẢNH 3D",
    "icon": "📁",
    "children": [
      {
        "id": "prompt-couple",
        "name": "Prompt Couple",
        "icon": "💞",
        "children": []
      },
      {
        "id": "prompt-anh-don-boy",
        "name": "Prompt Ảnh Đơn - Boy",
        "icon": "👦",
        "children": []
      },
      {
        "id": "prompt-anh-don-girl",
        "name": "Prompt Ảnh Đơn - Girl",
        "icon": "👧",
        "children": []
      },
      {
        "id": "prompt-wedding",
        "name": "Prompt Wedding",
        "icon": "💍",
        "children": []
      },
      {
        "id": "prompt-background",
        "name": "Prompt Background",
        "icon": "🌆",
        "children": []
      },
      {
        "id": "prompt-anh-nhom",
        "name": "Prompt Ảnh Nhóm",
        "icon": "👥",
        "children": []
      }
    ]
  },
  {
    "id": "mix-match-outfit-game",
    "name": "MIX & MATCH OUTFIT GAME",
    "icon": "👗",
    "children": [
      {
        "id": "style-girl",
        "name": "Style Girl",
        "icon": "👧",
        "children": [
          {
            "id": "cute-girl",
            "name": "Cute Girl",
            "icon": "🌸",
            "children": []
          },
          {
            "id": "sexy-girl",
            "name": "Sexy Girl",
            "icon": "🔥",
            "children": []
          },
          {
            "id": "cool-girl",
            "name": "Cool Girl",
            "icon": "💎",
            "children": []
          },
          {
            "id": "style-105-d8-girl",
            "name": "Style 105 D8",
            "icon": "✨",
            "children": []
          }
        ]
      },
      {
        "id": "style-boy",
        "name": "Style Boy",
        "icon": "👦",
        "children": [
          {
            "id": "cute-boy",
            "name": "Cute Boy",
            "icon": "🌸",
            "children": []
          },
          {
            "id": "sexy-boy",
            "name": "Sexy Boy",
            "icon": "🔥",
            "children": []
          },
          {
            "id": "cool-boy",
            "name": "Cool Boy",
            "icon": "💎",
            "children": []
          },
          {
            "id": "style-105-d8-boy",
            "name": "Style 105 D8",
            "icon": "✨",
            "children": []
          }
        ]
      },
      {
        "id": "couple-outfit",
        "name": "Couple Outfit",
        "icon": "❤️",
        "children": [
          {
            "id": "cute-style",
            "name": "Cute Style",
            "icon": "🌸",
            "children": []
          },
          {
            "id": "sexy-style",
            "name": "Sexy Style",
            "icon": "🔥",
            "children": []
          },
          {
            "id": "cool-style",
            "name": "Cool Style",
            "icon": "💎",
            "children": []
          }
        ]
      }
    ]
  },
  {
    "id": "video-game-audition",
    "name": "VIDEO GAME AUDITION",
    "icon": "🎬",
    "children": [
      {
        "id": "mv-audition",
        "name": "MV Audition",
        "icon": "🎵",
        "children": []
      },
      {
        "id": "perfect-combo-audition",
        "name": "Perfect x Combo Audition",
        "icon": "🏆",
        "children": []
      },
      {
        "id": "d8-skill-dance-performance",
        "name": "D8 SKILL DANCE PERFORMANCE",
        "icon": "💃",
        "children": [
          {
            "id": "mua-quat",
            "name": "Múa Quạt",
            "icon": "🪭",
            "children": []
          },
          {
            "id": "poppin",
            "name": "Poppin",
            "icon": "🔥",
            "children": []
          },
          {
            "id": "d8-sexy-girl",
            "name": "D8 Sexy Girl",
            "icon": "💃",
            "children": []
          },
          {
            "id": "d8-cool-girl",
            "name": "D8 Cool Girl",
            "icon": "💎",
            "children": []
          },
          {
            "id": "d8-sexy-boy",
            "name": "D8 Sexy Boy",
            "icon": "🕺",
            "children": []
          },
          {
            "id": "d8-cool-boy",
            "name": "D8 Cool Boy",
            "icon": "🧢",
            "children": []
          }
        ]
      },
      {
        "id": "d8-team-dance-performance",
        "name": "D8 TEAM DANCE PERFORMANCE",
        "icon": "👯",
        "children": [
          {
            "id": "couple",
            "name": "COUPLE",
            "icon": "💞",
            "children": []
          },
          {
            "id": "girl-girl",
            "name": "Girl & Girl",
            "icon": "👭",
            "children": []
          },
          {
            "id": "boy-boy",
            "name": "Boy & Boy",
            "icon": "👬",
            "children": []
          }
        ]
      },
      {
        "id": "doi-8-4k-dance-performance",
        "name": "ĐÔI 8-4K DANCE PERFORMANCE",
        "icon": "💞",
        "children": [
          {
            "id": "doi-8k",
            "name": "Đôi 8K",
            "icon": "8️⃣",
            "children": []
          },
          {
            "id": "doi-4k",
            "name": "Đôi 4K",
            "icon": "4️⃣",
            "children": []
          }
        ]
      },
      {
        "id": "d8-skill-review",
        "name": "D8 SKILL REVIEW",
        "icon": "⭐",
        "children": []
      },
      {
        "id": "dc8-skill-review",
        "name": "DC8 SKILL REVIEW",
        "icon": "⭐",
        "children": []
      }
    ]
  },
  {
    "id": "tam-su-chia-se",
    "name": "TÂM SỰ - CHIA SẺ",
    "icon": "💌",
    "children": []
  }
];

export const MINA_DEFAULT_TAGS = [
  "Audition",
  "D8",
  "Mina",
  "Ảnh 3D",
  "Review Skill",
  "Mix & Match",
  "Prompt",
  "D8Team"
];

export function cloneMinaCategories(value = MINA_DEFAULT_CATEGORIES) {
  return JSON.parse(JSON.stringify(value));
}

export function flattenMinaCategories(nodes = [], parentPath = []) {
  let result = [];

  nodes.forEach(node => {
    const path = [...parentPath, node.name];

    result.push({
      ...node,
      path,
      fullName: path.join("/")
    });

    if (Array.isArray(node.children) && node.children.length) {
      result = result.concat(flattenMinaCategories(node.children, path));
    }
  });

  return result;
}
