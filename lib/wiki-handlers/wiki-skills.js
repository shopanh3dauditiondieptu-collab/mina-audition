...
function skillIdentifiers(skill = {}) {
  return [skill.id, skill.skillCode, skill.name, skill.legacyId, skill.sourceId]
    .map(cleanId)
    .filter(Boolean);
}

function findSkillIndex(skills, requestedId) {
  const id = cleanId(requestedId);
  if (!id) return -1;
  return skills.findIndex(item => skillIdentifiers(item).includes(id));
}
...
function normalizeSkill(input = {}, previous = null) {
  ...
  return {
    ...
    name,
    skillCode: id,
    legacyId: String(input.legacyId ?? previous?.legacyId ?? "").trim(),
...
async function mutateDatabase(mutator, commitMessage, attempts = 3) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    const next = {
      ...data,
      version: Math.max(13, Number(data.version) || 1),
      updatedAt: new Date().toISOString(),
      skills: result.skills,
      trash: result.removed
        ? [
            ...(Array.isArray(data.trash) ? data.trash : []),
            { ...result.removed, deletedAt: new Date().toISOString() }
          ].slice(-100)
        : (Array.isArray(data.trash) ? data.trash : []),
      history: [
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const output = await mutateDatabase(data => {
        ...
        const duplicateIndex = findSkillIndex(data.skills, skill.id);
        if (duplicateIndex >= 0) {
          const error = new Error(`Skill ${skill.id} đã tồn tại. Hãy dùng chức năng Sửa.`);
      ...
      }, result => `Add skill ${result.skill.id} - ${result.skill.name}`);
    ...
    if (req.method === "PUT") {
      ...
      const originalId = String(
        req.body?.originalId || raw.originalId || req.query?.originalId || raw.id || raw.skillId || ""
      ).trim();
      if (!originalId) return sendJson(res, 400, { ok: false, error: "Thiếu ID gốc của Skill" });

      const output = await mutateDatabase(data => {
        const index = findSkillIndex(data.skills, originalId);
        if (index < 0) {
          const error = new Error(`Không tìm thấy Skill ${originalId}`);
        ...
        if (duplicateIndex >= 0 && duplicateIndex !== index) {
          const error = new Error(`Không thể đổi mã: Skill ${skill.id} đã tồn tại.`);
          error.statusCode = 409;
          throw error;
        }
        ...
        skills[index] = skill;
        return { skills, skill, originalId };
      }, result => result.originalId === result.skill.id
        ? `Update skill ${result.skill.id}`
        : `Rename skill ${result.originalId} to ${result.skill.id}`);
    ...
    if (req.method === "DELETE") {
      ...
      const output = await mutateDatabase(data => {
        const index = findSkillIndex(data.skills, id);
        if (index < 0) {
