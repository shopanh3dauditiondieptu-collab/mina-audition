...
/* =========================================================
   MINA WIKI SKILLS HANDLER V4.2 — SAFE DB & IMAGE SYNC
   File đích: lib/wiki-handlers/wiki-skills.js

   Nguồn chính: public/database/master-skills.json; tự đồng bộ database/master-skills.json
...
const MIRROR_DB_PATH = process.env.MINA_MIRROR_DB_PATH || "database/master-skills.json";
const DB_VERSION = 15;
...
function contentUrl(path = DB_PATH) {
  const { owner, repo } = repoConfig();
  return `${GH_API}/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${path}`;
}
...
function normalizeSkill(input = {}, previous = null) {
  ...
  /*
   * Đồng bộ mọi tên trường ảnh cũ/mới về imageUrl.
   * Khi sửa Skill mà không chọn ảnh mới, ảnh cũ vẫn được giữ nguyên.
   */
...
async function readDatabaseAtPath(path) {
  const { branch } = repoConfig();
  const response = await fetch(
    `${contentUrl(path)}?ref=${encodeURIComponent(branch)}&t=${Date.now()}`,
    {
      method: "GET",
  ...
  if (!response.ok) {
    throw new Error(
      `GitHub GET ${path} lỗi ${response.status}: ${await response.text()}`
    );
  }
...
async function readDatabase() {
  return readDatabaseAtPath(DB_PATH);
}

async function writeDatabaseAtPath(path, data, sha, message) {
  const { branch } = repoConfig();
  const body = {
    message,
    branch,
  ...
  if (!response.ok) {
    const error = new Error(
      `GitHub PUT ${path} lỗi ${response.status}: ${await response.text()}`
    );
...
async function writeDatabase(data, sha, message) {
  /*
   * Nguồn chính mà Website/CMS dùng.
   */
  const primaryResult = await writeDatabaseAtPath(
    DB_PATH,
    data,
    sha,
    message
  ...
  /*
   * Đồng bộ thêm bản mirror ở database/master-skills.json.
   * Nếu hai biến môi trường trỏ cùng một file thì không ghi lần hai.
   */
  if (MIRROR_DB_PATH && MIRROR_DB_PATH !== DB_PATH) {
    const mirror = await readDatabaseAtPath(MIRROR_DB_PATH);

    await writeDatabaseAtPath(
      MIRROR_DB_PATH,
      data,
      mirror.sha,
      `${message} [sync mirror]`
...
function publicSkill(skill) {
  return {
    ...
    thumbnail: skill.imageUrl || "",
    thumbnailUrl: skill.imageUrl || "",
    cover: skill.imageUrl || "",
    coverUrl: skill.imageUrl || "",
    thumb: skill.imageUrl || "",
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === "PUT") {
      ...
      const originalSkillCode = cleanText(
        req.body?.originalSkillCode ||
        raw.originalSkillCode ||
        req.query?.originalSkillCode
      );
      const incomingSkillCode = inferSkillCode(raw, null);

      if (!internalId && !originalSkillCode && !incomingSkillCode) {
        return sendJson(res, 400, {
          ok: false,
          error: "Thiếu ID hệ thống hoặc mã Skill cần cập nhật"
        });
      }

      const output = await mutateDatabase(data => {
        let index = findByInternalId(data.skills, internalId);

        /*
         * Tương thích dữ liệu đã migration:
         * CMS có thể đang giữ internalId cũ dạng wiki_xxx trong khi database
         * hiện tại đã chuẩn hóa về skill_<mã>. Khi đó không được báo 404 ngay,
         * mà tìm lại bản ghi bằng mã Skill gốc rồi cập nhật trên cùng một record.
         */
        if (index < 0 && originalSkillCode) {
          index = findBySkillCode(data.skills, originalSkillCode);
        }
        if (index < 0 && incomingSkillCode) {
          index = findBySkillCode(data.skills, incomingSkillCode);
        }

        if (index < 0) {
          const lookup = [
            internalId ? `ID hệ thống ${internalId}` : "",
            originalSkillCode ? `mã gốc ${originalSkillCode}` : "",
            incomingSkillCode ? `mã Skill ${incomingSkillCode}` : ""
          ].filter(Boolean).join(", ");
          const error = new Error(`Không tìm thấy Skill theo ${lookup}`);
          error.statusCode = 404;
          throw error;
        }

        const previous = data.skills[index];
        const skill = normalizeSkill({ ...raw, internalId: previous.internalId || previous.id }, previous);
        const duplicateIndex = findBySkillCode(data.skills, skill.skillCode);
