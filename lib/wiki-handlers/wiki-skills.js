    ...
    if (req.method === "POST") {
      ...
      const output = await mutateDatabase(data => {
        const requestedId = String(
          raw.id || raw.skillId || raw.skillCode || raw.name || ""
        ).trim().toLowerCase();

        const index = data.skills.findIndex(item =>
          String(item.id).trim().toLowerCase() === requestedId ||
          String(item.skillCode || item.name || "").trim().toLowerCase() === requestedId
        );
